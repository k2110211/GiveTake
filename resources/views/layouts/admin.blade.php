    ...
    <aside class="w-64 bg-gray-900 flex flex-col flex-shrink-0">
        ...
        <div class="h-16 flex items-center px-6 border-b border-gray-700/50">
            <a href="{{ route('admin.dashboard') }}" class="flex items-center space-x-2.5">
                ...
                <span class="text-white font-bold text-sm tracking-wide">Cho & Nhận Admin</span>
            </a>
        </div>
        ...
        <nav class="flex-1 py-6 px-3 space-y-1 overflow-y-auto">
            @php
                $pendingItemsCount = \App\Models\Item::where('item_status_id', \App\Models\ItemStatus::PENDING)->count();
                $navItems = [
                    ...
                    ['route' => 'admin.items',        'label' => 'Món đồ',           'icon' => 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4', 'badge' => $pendingItemsCount],
                ...
                <a href="{{ route($item['route']) }}"
                   class="flex items-center px-3 py-2.5 rounded-xl text-sm font-medium transition-all group
                    ...
                    @if(!empty($item['badge']) && $item['badge'] > 0)
                        <span class="ml-2 px-2 py-0.5 rounded-full text-xs font-extrabold bg-amber-500 text-white animate-pulse">
                            {{ $item['badge'] }}
                        </span>
                    @endif
                </a>
